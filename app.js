...
(function () {
  ...
  const els = {
    ...
    overflowBtn: document.getElementById("btnOverflow")
  };
  ...
  const scenarios = {
    ...
    configPrinter: {
      label: "Configurar impresora de etiquetas",
      receiptType: "Recibo simple",
      printingByBoxes: true,
      lpnConfig: true,
      printer: { mode: "config" },
  ...
  function reset(scenarioKey, lpn) {
    ...
    state = {
      ...
      selectedPrinter: "",
      menuOpen: scenarioKey === "configPrinter"
    };
    if (scenarioKey === "configPrinter") {
      state.stage = "configMenu";
    }
  ...
  function resolvePrinter(scenario, generated) {
    ...
    if (printer.mode === "config") {
      sendToPrinter(generated.total, state.selectedPrinter || "impresora de zona");
      return;
    }
  ...
  function printerPickerHtml(opts) {
    opts = opts || {};
    var copy = "copy" in opts
      ? opts.copy
      : "Para realizar la impresión de etiquetas, primero debes seleccionar una impresora";
    var info = opts.info || "";
    ...
    var canPrint = !required || !!state.selectedPrinter;
    var copyHtml = copy ? '<p class="modal-copy">' + copy + "</p>" : "";
    var infoHtml = info
      ? '<p class="modal-info"><i class="pi pi-info-circle" aria-hidden="true"></i><span>' + info + "</span></p>"
      : "";
    return (
      ...
      '<div class="modal" id="printerModal" role="dialog" aria-modal="true" aria-labelledby="printerModalTitle">' +
        ...
        '<div class="field" style="margin:16px 0 0">' +
          infoHtml +
          '<label class="field-label" for="printerSelect">Seleccionar impresora' +
  ...
  function overflowMenuHtml() {
    return (
      '<div class="overflow-menu" id="overflowMenu" role="menu" aria-label="Configuración de la terminal">' +
        '<button class="overflow-item" type="button" role="menuitem" tabindex="-1">Cambiar de cuenta</button>' +
        '<button class="overflow-item" type="button" role="menuitem" tabindex="-1">Configuración de la terminal</button>' +
        '<button class="overflow-item is-action" id="btnSelectPrinter" type="button" role="menuitem">Seleccionar impresora</button>' +
        '<button class="overflow-item" type="button" role="menuitem" tabindex="-1">Cerrar sesión</button>' +
      "</div>"
    );
  }

  function printerToast(text) {
    els.toastText.innerHTML = text;
    els.toast.classList.add("is-success");
    els.toast.hidden = false;
    window.clearTimeout(printerToast.timer);
    printerToast.timer = window.setTimeout(function () {
      els.toast.hidden = true;
  ...
  function bindOverflowMenu() {
    if (els.overflowBtn) {
      els.overflowBtn.classList.toggle("is-open", !!state.menuOpen);
      els.overflowBtn.setAttribute("aria-expanded", state.menuOpen ? "true" : "false");
    }
    var pick = document.getElementById("btnSelectPrinter");
    if (!pick) return;
    pick.addEventListener("click", function () {
      state.menuOpen = false;
      state.stage = "configPickPrinter";
      render();
    });
  }
  ...
  function render() {
    ...
    } else if (state.stage === "configMenu") {
      els.stepList.innerHTML = "";
      els.promptZone.innerHTML = "";
      if (state.menuOpen) {
        modalHost().innerHTML = overflowMenuHtml();
        bindOverflowMenu();
      }
    } else if (state.stage === "configPickPrinter") {
      els.stepList.innerHTML = "";
      els.promptZone.innerHTML = "";
      modalHost().innerHTML = printerPickerHtml({
        copy: "",
        info: "Si no eliges una impresora, se usará la opción predeterminada para la zona de trabajo.",
        required: false
      });
      bindConfigPicker();
  ...
  function bindConfigPicker() {
    ...
    confirm.addEventListener("click", function () {
      ...
      state.menuOpen = false;
      render();
      printerToast("La impresora ha sido seleccionada correctamente.");
    });
    close.addEventListener("click", function () {
      clearModal();
      state.stage = "configMenu";
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (state.stage === "configPickPrinter") {
      event.preventDefault();
      clearModal();
      state.stage = "configMenu";
      state.menuOpen = true;
  ...
  if (els.overflowBtn) {
    els.overflowBtn.addEventListener("click", function () {
      if (state.stage === "configPickPrinter") {
        clearModal();
        state.stage = "configMenu";
      }
      if (state.scenarioKey !== "configPrinter" && state.stage !== "configMenu") {
        return;
      }
